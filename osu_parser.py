    ...
    for section in sections:
        ...
        while t_aux <= 1 and total_distance < length:
            ...
            if (degree == 1):
                ...
                bezierX = pow(1 - t_aux, 2) * section[0][0] + 2 * (1 - t_aux) * t_aux * section[1][0] + pow(t_aux, 2) * \
                          section[2][0]
                bezierY = pow(1 - t_aux, 2) * section[0][1] + 2 * (1 - t_aux) * t_aux * section[1][1] + pow(t_aux, 2) * \
                          section[2][1]
            elif (degree == 3):
                bezierX = pow(1 - t_aux, 3) * section[0][0] + 3 * pow(1 - t_aux, 2) * t_aux * section[1][0] + 3 * (
                            1 - t_aux) * pow(t_aux, 2) * section[2][0] + pow(t_aux, 3) * section[3][0]
                bezierY = pow(1 - t_aux, 3) * section[0][1] + 3 * pow(1 - t_aux, 2) * t_aux * section[1][1] + 3 * (
                            1 - t_aux) * pow(t_aux, 2) * section[2][1] + pow(t_aux, 3) * section[3][1]
            else:
                for i in range(degree + 1):
                    bezierX += binomialCoeficient(degree, i) * pow(1 - t_aux, degree - i) * pow(t_aux, i) * section[i][
                        0]
                    bezierY += binomialCoeficient(degree, i) * pow(1 - t_aux, degree - i) * pow(t_aux, i) * section[i][
                        1]
    ...
    for ho in HOs:
        if ho.obj == 2:
            ...
            for count, point in enumerate(ho.path[:-1]):
                ...
                dist = math.sqrt(
                    math.pow(point[0] - ho.path[count + 1][0], 2) + math.pow(point[1] - ho.path[count + 1][1], 2))
                if dist > highest:
                    highest = dist
            ...
            for point in ho.path[1:]:
                ...
                dist = math.sqrt(
                    math.pow(new_point[0] - new_path[-1][0], 2) + math.pow(new_point[1] - new_path[-1][1], 2))
                if dist > highest * 0.9:
